...
- **Destinations map (Leaflet + OpenStreetMap)**: §6.3/§6.7 don't mention a map, but a visual map of proposed destinations was requested. Added `DestinationOption.latitude`/`longitude` (`Float?`, migration `20260705180143_add_destination_coordinates`) — a real schema change, confirmed with the user first per §1. Coordinates are populated automatically server-side in `proposeDestination` (`lib/services/voting.ts`) via `lib/services/geocoding.ts#geocodeLocation`, which geocodes the typed destination name through OSM's free Nominatim API (no API key/billing, vs. Google Maps which would need one) — chosen over having the proposer drop a manual pin, trading a small risk of a vague/misspelled name geocoding wrong for zero extra UI. Geocoding is best-effort: a failed/ambiguous lookup leaves `latitude`/`longitude` `null` and the proposal still succeeds, it just won't get a map pin. Rendering uses `react-leaflet`; `components/voting/DestinationsMap.tsx` is the actual Leaflet map (`"use client"`) and `components/voting/DestinationsMapLoader.tsx` wraps it in `next/dynamic` with `ssr: false`, since Leaflet touches `window` at import time and can't run during SSR — Server Component pages import the loader, never the map directly. Markers are custom `L.divIcon` badges showing vote count (bigger/green for the finalized-or-currently-leading destination) rather than the default Leaflet pin, which sidesteps the well-known bundler/marker-icon-path issue entirely. Shown on the Destinations page (all proposed options) and, per explicit user choice, also on the trip dashboard (highlighting the finalized destination once set, or the current vote leader before that) — hidden during `PLANNING` since there are no options to plot yet. Once a trip leaves `VOTING` (`FINALIZED`/`COMPLETED`/`ARCHIVED`), both maps filter down to only the winning `finalDestinationId` rather than plotting every proposed option — the losing options' `DestinationOption`/`DestinationVote` rows are left in the database (still shown in the Destinations page's card list underneath, and needed for `finalizeTrip`'s own history), only the *map* view narrows to the pick.
- **Known issue, not fixed**: Prisma's default query engine (`lib/db.ts`'s bare `new PrismaClient()`, no driver adapter) intermittently fails to reach the Neon database in local dev with `Can't reach database server`, even though a plain `pg` client connects to the exact same `DATABASE_URL`/`DIRECT_URL` reliably every time — confirmed by direct comparison during this debugging session. When it fires mid-request, Auth.js's session lookup treats the connection error as an invalid session and calls the adapter's `deleteSession`, signing the user out. The obvious fix is wiring up `@prisma/adapter-pg` (already a dependency) in `lib/db.ts`, but it's currently pinned to `^7.8.0` while `@prisma/client`/`prisma` are on `^6.19.3` — a real version mismatch that needs resolving (align both to the same major) before the adapter can be wired up safely. Left alone pending user direction since it's a dependency-version change, not a one-line fix.
  - **Resolved**: fixed in commit `f8f30ff` ("Upgrade to Prisma 7 and use @prisma/adapter-pg driver adapter") — `@prisma/client`/`prisma` moved to `^7.8.0` to match `@prisma/adapter-pg`, and `lib/db.ts` now constructs `PrismaClient` with `new PrismaPg({ connectionString: process.env.DATABASE_URL })` as its adapter. Left the original entry above intact as the record of what the bug was; noting the fix here rather than deleting the history.
- **Dashboard shell redesigned around a persistent left sidebar**: replaced the old top-nav-only `DashboardLayout`/`TripLayout` pair with `components/layout/AppShell.tsx` (Sidebar + TopBar + main), matching a supplied design mockup. Because Next.js layouts only receive `params` for their own segment and ancestors (not descendants — confirmed in `node_modules/next/dist/docs/01-app/03-api-reference/03-file-conventions/layout.md`), a single shared `layout.tsx` can't know `tripId` above the `[tripId]` segment. So `app/(dashboard)/layout.tsx` is now a bare session guard with no chrome, and `AppShell` is instantiated independently in two places: `app/(dashboard)/trips/[tripId]/layout.tsx` (trip-scoped nav: Destinations/Dates/Itinerary/Expenses/Budget/Members/Settings, gated the same way the old top-nav was) and directly in each generic page — `trips/page.tsx`, `trips/new/page.tsx`, `invites/page.tsx` — via the `lib/layout/shell.tsx#getGenericShellData()` helper (My Trips / Invites nav only). This is intentional duplication, not an oversight: nesting a shell-rendering `trips/layout.tsx` above `[tripId]/layout.tsx` would stack two sidebars once inside a trip.
  - **Sidebar nav items hold rendered icon elements, not component references**: `SidebarNavLinks.tsx` is a `"use client"` component; a Server Component passing `icon: HomeIcon` (a function) into its props array crashes at runtime with "Functions cannot be passed directly to Client Components" (RSC serialization boundary — only already-rendered JSX can cross it). `SidebarNavItem.icon` is typed `ReactNode` and every call site passes `icon: <HomeIcon />`, not the bare component.
  - **Icons and the mountain illustration were extracted for reuse**: `components/icons.tsx` (all the small line icons) and `components/illustrations/Mountain.tsx` (`MountainLogo`, `MountainScene`) were pulled out of the homepage (`app/page.tsx`) so the new sidebar/promo-card and the trip-overview hero could reuse them instead of duplicating the SVGs.
  - **Trip overview hero uses the generated `MountainScene` illustration, not a real photo**: there's no cover-image field on `Trip`/`DestinationOption` (no schema change made for this), so a fabricated stock photo would misrepresent stored data. The Leaflet destination map (`DestinationsMapLoader`, real geocoded pins) is kept as a separate section below, per the existing documented decision to show it on the dashboard.
  - **"Next Up" tasks are derived entirely from real state, not hardcoded**: while `VOTING`, a "Vote on destinations/dates" task appears only if the current user hasn't cast that vote yet (`votes.some(v => v.user.id === userId)`, same check `DestinationOptionCard`/`DateOptionCard` already use), with its avatar stack showing the *other* members who also haven't voted. Once `FINALIZED`, "Review budget" / "Add to itinerary" appear only if `categories.length === 0` / `itineraryItems.length === 0` respectively. Nothing renders as a fake "waiting on 3 people" placeholder.
  - **Destination/Dates stat cards distinguish "proposed, no votes yet" from "no proposals at all"**: the leading-option reducers (`leadingDestination`/`leadingDateOption`) fall back to the first proposed option (not `null`) when nobody has voted yet, so a real proposal with 0 votes shows its name with an "Awaiting votes" caption instead of incorrectly reporting "No proposals yet" (caught during Playwright verification — see below).
  - **Progress stat card is a fixed 6-step lifecycle checklist** (created → destinations proposed → dates proposed → finalized → budget set → itinerary started), computed inline as a local `steps` array on the overview page. The same 6 steps were later reimplemented in `lib/services/trips.ts#listUserTripsWithSummary` (as `progress.completed`/`progress.total`) for the trips-list redesign below, so both surfaces agree on what "done" means even though the two aren't sharing one function — `TripCard`'s earlier standalone `STATUS_PROGRESS` map (a cruder per-status percentage) was removed in favor of this real, step-based count.
  - **Members panel shows pending invitations, owner-only**: `lib/services/invitations.ts#listPendingTripInvitations` is new and gated by `requireTripOwner`, since invitee email addresses are exactly the kind of data §7's security checklist says shouldn't leak beyond who needs it — non-owners only ever see the accepted-members list.
  - **Budget Summary panel reports "Spent", not "Collected"**: the mockup's panel had a "Collected" figure implying members contribute funds toward a shared pot, which has no backing data model (`Expense` tracks money paid out, not money paid in). Relabeled to Total Budget / Spent / Remaining using the existing `getBudgetActuals` sum, and swapped the mockup's unbacked "Add Funds" button for a real "Add Expense" link to `/trips/[tripId]/expenses`.
  - **Recent Expenses / Settlement / itinerary-preview sections removed from the overview page**: superseded by the stat cards + Next Up + right rail, which link out to the same data on their respective sub-pages. Nothing is unreachable, the overview is just no longer a full data dump.
  - **Notification bell / chat icons from the mockup were omitted entirely**, not stubbed — there's no notifications or messaging feature to back them, and per house rule, decorative dead UI isn't shipped ahead of the feature it implies.
  - Verified by scripting the full lifecycle (sign up → create trip → start voting → propose → vote → finalize) through a headless Playwright session against the dev server rather than just eyeballing one static state, which is what caught the "Awaiting votes" bug above; `console --errors` was clean throughout.
- **Trips list page (`app/(dashboard)/trips/page.tsx`) redesigned to match a second supplied mockup** — richer trip cards (avatar stack, location, date range, per-trip progress), Upcoming/Past grouping, and an Invites preview rail. Two scope questions were resolved with the user up front rather than guessed: the sidebar stays My-Trips/Invites-only on this page (no fabricated "jump into trip-scoped nav with no trip selected" links), and each card's kebab (⋮) menu from the mockup was omitted entirely — Settings already covers owner edit/delete and there's no other backing action yet.
  - **`lib/services/trips.ts#listUserTripsWithSummary`** wraps `listUserTrips` and adds, per trip, a preview of up to 5 member avatars, the current/leading destination name and date range, and the 6-step progress count described above. Leading destination/dates while `VOTING` are resolved with `orderBy: { votes: { _count: "desc" } }` (Prisma relation-count ordering) rather than fetching every option's votes — cheaper than the overview page's approach, appropriate since the list page only needs one name/date pair per trip, not full vote-count displays.
  - **Upcoming vs. Past grouping is status-based**, not date-based: `PLANNING`/`VOTING`/`FINALIZED` → "Upcoming Trips", `COMPLETED`/`ARCHIVED` → "Past Trips" (see `lib/trip-status.ts`'s `UPCOMING_TRIP_STATUSES`/`PAST_TRIP_STATUSES`). Note there is still no in-app action that ever sets a trip to `COMPLETED`/`ARCHIVED` — those enum values exist in the schema but nothing transitions a trip into them yet, so "Past Trips" will stay empty for every real trip until that's built. Verified the section's rendering by writing the status directly in the dev database for a screenshot, not by adding a fake UI trigger.
  - **`lib/trip-status.ts`** centralizes `TRIP_STATUS_LABELS`/`TRIP_STATUS_BADGE_CLASSES` (`FINALIZED` reads as "Upcoming", matching the overview hero's existing wording) — both `TripCard` and the overview page import from here now instead of keeping their own copies.
  - **Invite preview cards use the same generated `MountainScene` illustration as trip cards** (in a circular crop), not a photo — consistent with the no-fabricated-photos rule elsewhere. **`TripInvitation.invitedBy` is a plain userId string, not a relation** (per the existing `proposedBy`-style plain-string precedent), so `lib/services/invitations.ts#listPendingInvitationsForUserPreview` resolves the inviter's display name with one small batched `User` lookup rather than a join.
  - **Trip card illustration gets a small deterministic hue-rotate per trip id** (`hueShift()` in `TripCard.tsx`, hashed from the id, ±15deg) purely so a list of several cards isn't visually identical — decorative only, not meant to imply per-trip photos.
